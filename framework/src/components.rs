...
pub fn scan_components(dir: &Path) -> std::io::Result<Vec<Component>> {
    let mut components_map: HashMap<String, (Option<PathBuf>, Option<PathBuf>)> = HashMap::new();
    ...
    let components = components_map
        .into_iter()
        .filter_map(|(id, (logic_path, template_path))| {
            template_path.map(|template_path| Component {
                id,
